...
def load_datasets(data_dir, augment):
    transforms = _get_transforms(augment)
    trainset   = torchvision.datasets.ImageFolder(os.path.join(data_dir, 'train'), transforms['training'])
    validset   = torchvision.datasets.ImageFolder(os.path.join(os.path.realpath(data_dir), 'val'), transforms['validation'])
    testset    = validset
